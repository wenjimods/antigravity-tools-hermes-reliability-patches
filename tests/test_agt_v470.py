...
class V470(unittest.TestCase):
 def test_exact_source_and_roundtrip(self):
  ...
  with tempfile.TemporaryDirectory(prefix='agt 测试 ') as d:
   ...
   self.assertFalse((t/'src-tauri/src/proxy/refresh_budget.rs').exists())
 ...
 def test_helper_identity_and_semantics(self):
  m=json.loads((A/'manifest.json').read_text())
  self.assertIsNone(m['files']['src-tauri/src/proxy/refresh_budget.rs']['before']); self.assertFalse((A/'baseline/src-tauri/src/proxy/refresh_budget.rs').exists()); self.assertEqual(digest((A/'refresh_budget.rs').read_bytes()),m['files']['src-tauri/src/proxy/refresh_budget.rs']['after'])
