...
从工具包根目录运行 `python scripts/apply_hermes_patch.py /path/to/source --dry-run`；去掉 `--dry-run` 才写入。应用器在临时目录真实应用补丁并核验全部输出哈希，未知版本/部分修改不写，完整已应用状态重复执行直接成功。只约束目标文件；不要求其他无关文件 clean。写入异常尝试恢复原文件，但断电中断不具备跨文件事务保证，因此只用于隔离副本。
